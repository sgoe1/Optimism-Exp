...
with open('index_extract.csv', 'wb') as csvfile:
    cleanedwriter = csv.writer(csvfile, delimiter=',',quotechar='|', quoting=csv.QUOTE_MINIMAL)
    cleanedwriter.writerow(['Participant', 'Assignment ID', 'Completion Time (Min)', 'Condition', 'Time Horizon', 'Number of investments',
                            'Final Rate of Dev.', 'Money at end of game', 'Est. rate of dev. for investing',
                            'Est. rate of dev. for marketing', 'Bonus ($)'])

    with open('resources/MainExperiment.csv', 'rU') as data_file:
